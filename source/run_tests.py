...
from shape_generators import SG1, SG2, SG3, SG4, SG5, SG6, SG7, FromCSVFileShapeGenerator
...
for name, placer in racers.items():
    ...
    if best_run:
        ...
        with open('../results/' + picking_policy + '_' + str(radius) + '.txt', 'w') as f:
            for cont in containers:
                ...
                for i, o in enumerate(SFG_competition):
                    ...
                    sg = mp.run(CFR_mode, choose_least_CFR_area, picking_policy)
                ...
                print(
                    f"Used picking policy: {picking_policy}, used container: {container}, used CFR_mode: {CFR_mode}, used choose_least_CFR_area: {choose_least_CFR_area}")
                print(
                    f"**************************************************************************************************************************")

                print(f"{picking_policy}, {CFR_mode}, {choose_least_CFR_area}, {container}", file=f)
                for score in filled:
        ...
        for i in range(len(filenames)):
            ...
            mp = placer(FromCSVFileShapeGenerator(polygon_radius=radius, rotations=rotation, filename=filename, container=cont, scale=scale), cont)
            sg = mp.run(CFR_mode, choose_least_CFR_area, picking_policy)
        ...
        with open('../results/' + picking_policy + '.txt', 'w') as f:
            for cont in containers:
                for CFR_mode in [0, 1, 2]:
                    for choose_least_CFR_area in [False, True]:
                        filled = []
                        shapes = []
                        times = []
                        for i, o in enumerate(SFG_competition):
                        ...
                        print(f"Total time taken: {sum(times)}s, that is {sum(times) / sum(shapes)}s per shape")
                        container = "circle" if cont is None else "square"
                        print(f"Used picking policy: {picking_policy}, used container: {container}, used CFR_mode: {CFR_mode}, used choose_least_CFR_area: {choose_least_CFR_area}")
                        print(f"**************************************************************************************************************************")
