...
/**
 * Remove co-linear points from a polygon. Don't correct because it's the inside of IFP
 * @param p A reference to a polygon.
 */
void remove_co_linear_ifp(polygon_t& p) {
	remove_co_linear(p.outer());
	for (auto& r : p.inners())
		remove_co_linear(r);
}
...
nfp_t generate_ifp(polygon_t& pA, polygon_t& pB, const bool checkValidity = true) {
	remove_co_linear_ifp(pA);
	remove_co_linear_ifp(pB);
    ...
    while (true) {
        SearchStartResult res = search_start_translation(pA.outer(), pB.outer(), ifp, true, startTrans);
        if (res == FOUND) {
            ifp.push_back( { });
            DEBUG_VAL("##### hole start #####");
