...
//#define NFP_DEBUG
#define FILENAMEPREFIX "this_kind_"
...
void write_svg(std::string const& filename,const std::vector<segment_t>& segments) {
	std::string const& filename2 = FILENAMEPREFIX + filename;
    std::ofstream svg(filename2.c_str());
...
void read_wkt_polygon2(const string str, polygon_t& p) {
	bg::read_wkt(str, p);
	bg::correct(p);
}
...
SlideResult slide(polygon_t& pA, polygon_t::ring_type& rA, polygon_t::ring_type& rB, nfp_t& nfp, const point_t& transB, bool inside) {
	...
	int max_try = 5000;
	while(startAvailable && max_try-- > 0) {
		DEBUG_VAL(cnt);
...
nfp_t generateIFP(polygon_t& pA, polygon_t& pB, const bool checkValidity = true) {
	removeCoLinear(pA);
	removeCoLinear(pB);
	...
	while(true) {
		SearchStartResult res = searchStartTranslation(pA.outer(), pB.outer(), ifp, true, startTrans);
		if(res == FOUND) {
			ifp.push_back({});
			DEBUG_VAL("##### 'hole' start #####");
			slide(pA, pA.outer(), pB.outer(), ifp, startTrans, true);
			DEBUG_VAL("##### 'hole' end #####");
		} else if(res == FIT) {
	  		point_t reference = pB.outer().front();
	  		point_t translated;
			trans::translate_transformer<coord_t, 2, 2> translate(startTrans.x_, startTrans.y_);
			boost::geometry::transform(reference, translated, translate);
			if(!inNfp(translated,ifp)) {
				ifp.push_back({});
				ifp.back().push_back(translated);
			}
			break;
		} else
			break;
